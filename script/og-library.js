...
$(function () {
    ...
    $(".featured.resources ul li").each(function () {
        ...
        function success(resp) {
            ...
            var tags = $(resp).find('div[id*="DetailPanel"] .row:nth-child(2) .col-md-10 .col-md-12 .user-content-hashtag');
            ...
            $(self).find('.img-container').append($('<span class="likes-display">' + likes + '</span>'));

            $(self).find('.img-container').prepend($(tags));
    ...
    $('.library-list').each(function () {
        ...
        var resourceImg = $(this).find('div[id*="NameBlock"] div:nth-of-type(4) .row .col-md-12 img');

        var resourceImgSrc = $(resourceImg).attr('src');

        $(img).css('background-image', 'url("' + resourceImgSrc + '")');

        var itemRating = $(this).find('.ItemRatingCommentPanel');

        $(img).append($(itemRating));

        var tagging = $(this).find('.user-content-hashtag');
    ...
    // handle filtering
    $('div[id*="DocumentPanel"]').prepend('<div class="dropdown-group"><div class="HtmlContent"></div></div>');
