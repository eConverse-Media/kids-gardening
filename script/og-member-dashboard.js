$(function () {
    // First Dashboard row
    $('.dashboard-name').wrap('<div class="dashboard-row-1 row" />');

    $('.dashboard-row-1').prepend($('#Welcome_Content .panel-body a[id*="Welcome_Details_ProfileLink"]'));
    ...
    // $('<span class="greeting">Welcome back,</span><br>').insertBefore($('.dashboard-name h4'));

    var dashboardNameText = $('.dashboard-name h2 span').text();

    // $('.dashboard-name h2 span').replaceWith('<a href="' + $('.dashboard-row-1 a').attr('href') + '">' + dashboardNameText + '</a>');
